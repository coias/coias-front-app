...
function DataSelector({ setFileNames }) {
  ...
  const [selectTractMode, setSelectTractMode] = useState(true);
  ...
  const [tractIds, setTractIds] = useState([]);
  useEffect(async () => {
    const res = await axios.get(`${reactApiUri}tract_list`).catch(() => {
      console.log('tract情報のロード時にエラーが発生しました');
    });
    if (res !== undefined) {
      setTractIds(Object.keys(res.data.result).map((key) => parseInt(key, 10)));
    }
  }, []);
  ...
  const tractOnClick = useCallback(
    async (tractIndex) => {
      if (!selectTractMode) return;
  ...
  return (
    <div style={{ flexGrow: 1 }}>
      <Row style={{ margin: '10px' }}>
        ...
        <Col>
          <Button
            onClick={() => setSelectTractMode(!selectTractMode)}
            className="btn-style box_blue"
          >
            {selectTractMode ? 'グラブモードへ' : '選択モードへ'}
          </Button>
        </Col>
        <Col>
          <h4>画像選択状況:</h4>
