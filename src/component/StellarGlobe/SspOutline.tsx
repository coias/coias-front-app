...
function SspOutline({ url, color = [0.25, 0.5, 1, 0.5] }: AreaLayerProps) {
...
async function loadPaths(url: string, color: V4): Promise<path.Path[]> {
  const { $any: region }: { $any: V3[][] } = await (await fetch(url))
    .json()
    .catch((e) => []);
  const width = 0.01;
  ...
  for (const piece of region) {
    const p: path.Path = {
      ...
      close: true,
      points: piece.map((v) => ({ position: v, color, size: width })),
    };
    paths.push(p);
  }
  return paths;
