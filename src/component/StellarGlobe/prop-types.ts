...
function makeVectorValidator({ size, isRequired }: { size: number, isRequired: boolean }) {
  return (props: any, propName: string, componentName: string) => {
    const value = props[propName]
    if (isRequired || (value === undefined)) {
      return null
    }
    if (Array.isArray(value) && value.length === size && value.every(c => typeof c === 'number')) {
      return null
    }
    return new Error(`invalid prop "${propName}" on ${componentName}. actual value=${JSON.stringify(value)}`)
  }
}
...
import {
  Patch, PatchSelector, Tract, TractSelector
} from './TractPatch'
